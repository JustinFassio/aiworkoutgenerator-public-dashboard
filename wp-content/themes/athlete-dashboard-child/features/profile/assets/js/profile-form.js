(function($) {
    ...
    class ProfileForm {
        ...
        initializeFormFields() {
            ...
            const $textareas = $('#profile-form textarea.auto-expand');
            ...
            $('.unit-selector').on('change', (e) => {
                ...
                $(document).trigger('profile_unit_changed', [field, $select.val()]);
            });
        }
        ...
        handleSubmit($form) {
            ...
            formData.append('profile_nonce', profileConfig.nonce);

            // Submit form via AJAX
            $.ajax({
                url: profileConfig.ajaxurl,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                success: (response) => {
                    if (response.success) {
                        ...
                        $(document).trigger('profile_updated', [response.data]);
                        
                        // Let the dashboard handle modal closing
                        $(document).trigger('dashboard_modal_close', ['profile-modal']);
                    } else {
